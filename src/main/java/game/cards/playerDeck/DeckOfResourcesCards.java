    ...
    int[] CountNumberOfEachResources() {
        ...
        for (ResourcesCard resourcesCard : resourcesCardsSet) {
            switch (resourcesCard.GetCardName()) {
                ...
                default ->
                        throw new IllegalArgumentException("Unrecognized resource type in DeckOfResourcesCards.resourcesCardsSet ");
            }
        }
        ...
        for (int i = 1; i < numberOfEachResource.length; i++) {
            if (numberOfEachResource[i] + numberOfGoldCoins >= numberOfCardsToRemove) {
                ...
                switch (i) {
                    ...
                    default ->
                            throw new IllegalArgumentException("Unable to retrieve back the resource name from the index in DeckOfResourcesCards.RemoveCardsOfSameResources");
                }
